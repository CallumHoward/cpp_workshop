...
std::shared_ptr<AvailableWorkshops> initialize(IA_UPTR& startTime, IA_UPTR& duration, size_t n);
int CalculateMaxWorkshops(const std::shared_ptr<AvailableWorkshops>& ptr);
...
std::shared_ptr<AvailableWorkshops> initialize(IA_UPTR& startTime, IA_UPTR& duration, size_t n) {
    auto availableWorkshops =
            std::make_shared<AvailableWorkshops>(AvailableWorkshops{});
