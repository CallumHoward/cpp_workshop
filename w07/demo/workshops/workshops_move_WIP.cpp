...
struct SmarterPointer {
    ...
    SmarterPointer(const SmarterPointer<T>& rhs) =delete;
    ~SmarterPointer() { delete p; }
    SmarterPointer<T> operator=(const SmarterPointer<T>&) =delete;
    SmarterPointer<T>& operator=(SmarterPointer<T>&& rhs) {
        p = std::move(rhs.p);
        return *this;
    }
...
int main() {
    ...
    SmarterPointer<AvailableWorkshops> ptr{std::move(initialize(startTime, duration, n))};
...
SmarterPointer<AvailableWorkshops>&& initialize(SmarterArray<int>& startTime,
    ...
    SmarterPointer<AvailableWorkshops> availableWorkshops{new AvailableWorkshops{}};
